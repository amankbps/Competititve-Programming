...
/*--------------------------------------------------------------------------------------------------------------------------*/
int dp[102][102][102];
int rec(string &a, string &b, string &c, int n, int m, int k)
{
    if (n < 0 || m < 0 || k < 0)
        return 0;
    if (dp[n][m][k] != -1)
        return dp[n][m][k];
    if (a[n] == b[m] && b[m] == c[k])
        return 1 + rec(a, b, c, n - 1, m - 1, k - 1);

    int ans = max({rec(a, b, c, n - 1, m, k), rec(a, b, c, n, m - 1, k), rec(a, b, c, n, m, k - 1)});
    return dp[n][m][k] = ans;
}
void solve()
{
